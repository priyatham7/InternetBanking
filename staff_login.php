...
if(isset($_REQUEST['submitBtn'])){
    ...
    $sql="SELECT username,password,staff_id FROM staff WHERE username='$username' AND password='$password'";
    $result=mysqli_query($conn,$sql);// or die(mysqli_error());
    $rws=  mysqli_fetch_array($result);
    echo $rws[0]."nooo";
    
    
    if($rws[0]==$username && $rws[1]==$password && $username!="" && $password!=""){
        session_start();
        $_SESSION['staff_login']=1;
        $_SESSION['staff_id']=$rws[0];
		$_SESSION['login_id']=$rws[2];
		header('location:staff_hompage.php'); 
    }
    else{
        echo "fail";
